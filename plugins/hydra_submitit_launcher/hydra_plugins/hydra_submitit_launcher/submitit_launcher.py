    ...
    def __call__(
        ...
        if snapshot_enabled:
            try:
                ...
                # Create snapshot (each job gets its own to avoid conflicts)
                # Add PID to make branch name unique and avoid race conditions
                job_branch_prefix = f"{branch_prefix}-job{job_num}-pid{os.getpid()}"
                log.debug(f"Creating git snapshot for job {job_num} (PID {os.getpid()})...")
                snapshot_branch, snapshot_commit = create_git_snapshot(
                    job_branch_prefix, repo_root
                )
                log.debug(
                    f"Created snapshot branch: {snapshot_branch} "
                    f"(commit: {snapshot_commit[:8]})"
                )
                ...
                snapshot_workdir = create_snapshot_worktree(
                    snapshot_branch, symlink_paths, repo_root, worktree_dir
                )
