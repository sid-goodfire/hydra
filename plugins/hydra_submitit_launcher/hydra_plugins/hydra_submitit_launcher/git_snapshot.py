...
def create_snapshot_worktree(
    ...
    Args:
        ...
        repo_root: Repository root path (auto-detected if None)
        worktree_dir: Base directory for worktree (auto-detected if None).
                     If None, creates in parent directory of repo root.
                     If specified, creates subdirectory there.
    ...
    # Determine base directory for worktree
    if worktree_dir is None:
        # Default: create in parent directory of repo root
        # e.g., if repo is /home/user/myproject, use /home/user/
        base_dir = repo_root.parent
    else:
        base_dir = Path(worktree_dir)
        base_dir.mkdir(parents=True, exist_ok=True)

    # Create temporary directory for the snapshot worktree
    temp_dir = Path(tempfile.mkdtemp(prefix="hydra-job-", dir=base_dir))
