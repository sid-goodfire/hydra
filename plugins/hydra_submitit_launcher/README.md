...
**`cleanup_after_job`** (bool, default: `false`)
- Whether to automatically delete snapshot worktree after job completes
- If `true`: Worktree deleted in job's finally block (runs on success or failure)
- If `false`: Worktrees remain for debugging/inspection (manual cleanup needed)
- **Note**: Snapshot git branches are never auto-deleted, only worktree directories
...
# Enable automatic cleanup after jobs complete
python my_app.py -m task=1,2,3 hydra.launcher.snapshot.cleanup_after_job=true

# Disable cleanup for debugging (keep worktrees for inspection)
python my_app.py -m task=1,2,3 hydra.launcher.snapshot.cleanup_after_job=false
```
